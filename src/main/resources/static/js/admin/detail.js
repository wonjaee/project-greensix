	...
	if (parentNo == 0) {
		$.ajax({
			...
			success: function(result) {
				...
				var pno=$(".category").siblings(".pno").val()
				$(".category > select").val(pno).prop("selected",true)
				categoryShow($(".category > select"))
			}
		})
	}
	else if(tagVal!=null) {
		$.ajax({
			...
			success: function(result) {
				...
				var pno=$(".category").siblings(".pno").val()
				var cno=$(".category").siblings(".cno").val()
				if(pno===tagVal){
					$(".category2 > select").val(cno).prop("selected",true)
				}
			}
		})
	}
